...
export default function List({ state, dispatch }) {
    return (
        <tr style={{ width:'100%',color: state.complete ? 'white' : 'black',backgroundColor:state.complete?'green':'LightGray' }}>
      
           
          <td style={{width:'150px',display: 'inline-block', wordWrap: 'break-word',  float: 'left',}}> {state.name} </td> 
